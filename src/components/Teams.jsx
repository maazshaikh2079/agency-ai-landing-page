...
const Teams = () => {
  return (
    <div className="border flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-800 dark:text-white">
      <Title
        title="Meet the team"
        description="A passionate team of digital experts dedicated to your brands success."
      />

      <div className="border grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
        {teamData.map((team, index) => (
          ...
          >
            ...
            <div className="border flex-1">
              <h3 className="border font-bold text-sm">{team.name}</h3>
              <p className="border text-xs opacity-60">{team.title}</p>
            </div>
          </div>
        ))}
      </div>
